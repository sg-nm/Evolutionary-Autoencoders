        ...
        if dataset_name == 'svhn' or dataset_name == 'celebA' or dataset_name == 'cars':
            if dataset_name == 'svhn':
                ...
                if self.validation:
                    ...
                    dataset = dset.ImageFolder(root='/home/suganuma/dataset/celebA_iccv_/train', transform=data_transform)
                    test_dataset = dset.ImageFolder(root='/home/suganuma/dataset/celebA_iccv_/val', transform=test_data_transform)
                else:
                    data_transform = transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor()])
                    test_data_transform = transforms.Compose([transforms.ToTensor()])
                    dataset = dset.ImageFolder(root='/home/suganuma/dataset/celebA_iccv_/train', transform=data_transform)
                    test_dataset = dset.ImageFolder(root='/home/suganuma/dataset/celebA_iccv_/test', transform=test_data_transform)
                self.dataloader = torch.utils.data.DataLoader(dataset, batch_size=self.batchsize, shuffle=True, num_workers=int(num_work), drop_last=True)
                self.test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=int(num_work))
                ...
                if self.validation:
                    data_transform = transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor()])
                    test_data_transform = transforms.Compose([transforms.ToTensor()])
                    dataset = dset.ImageFolder(root='/home/suganuma/dataset/cars/training/train', transform=data_transform)
                    test_dataset = dset.ImageFolder(root='/home/suganuma/dataset/cars/training/val', transform=test_data_transform)
                else:
                    data_transform = transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor()])
                    test_data_transform = transforms.Compose([transforms.ToTensor()])
                    dataset = dset.ImageFolder(root='/home/suganuma/dataset/cars/retrain/train', transform=data_transform)
                    test_dataset = dset.ImageFolder(root='/home/suganuma/dataset/cars/retrain/test', transform=test_data_transform)
                self.dataloader = torch.utils.data.DataLoader(dataset, batch_size=self.batchsize, shuffle=True, num_workers=int(num_work), drop_last=True)
                self.test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=int(num_work))
